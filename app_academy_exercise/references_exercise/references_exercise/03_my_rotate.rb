...
def my_rotate!(arr, amt)
    # 这个写法不可行，因为swap的时候最好ele的数量是等同的，否则会出现少element的情况
    # if amt > 0
    #     move = arr[0...amt]
    #     stay = arr[amt..-1]
    #     arr[0...amt], arr[amt..-1] = arr[amt..-1], arr[0...amt]
    # elsif 
    #     move = arr[amt..-1]
    #     stay = arr[0..amt-1]
    #     arr[0..amt-1], arr[amt..-1] = arr[amt..-1], arr[0..amt-1]
    # end
    # arr 
    if amt > 0
        amt.times do 
    ...
    ## 注意，循环里的操作如果不改变reference，那么循环就不会改变reference。
    ## 注意 基本上pop、shift的操作是不该被reference的，因为他的操作把value和index一起改变了。
    arr
end

array_1 = ["a", "b", "c", "d"]
p array_1.object_id                 # => 70354216023780
result_1 = my_rotate!(array_1, 1)
p result_1                          # => ["c", "d", "a", "b"]
